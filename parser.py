...
class Parser(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system, attention_type="affine"):
        ...
        if args.lstm == 'lstm':
            ...
            self.encoder_gru = nn.GRU(args.embed_size, int(args.hidden_size / 2), bidirectional=True)
            ...
            # self.decoder_gru = nn.GRU(input_dim, args.hidden_size)
            self.decoder_lstm = nn.LSTMCell(input_dim, args.hidden_size)
        else:
    ...
    def encode(self, src_sents_var, src_sents_len):
        ...
        if self.encoder_lstm is None:
            src_encodings, last_cell = self.encoder_gru(packed_src_token_embed)
        else:
            src_encodings, last_cell = self.encoder_lstm(packed_src_token_embed)
