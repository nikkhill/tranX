...
class Encoder(nn.Module):
    ...
    def __init__(self, position, layer, N):
...
class Parser(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system, attention_type="affine"):
        ...
        if args.encoder == 'transformer':
            ...
            # d_model should be args.embed_size (which is 128) instead of model default 512
            d_ff = 2048
            dropout = 0.1
            N = 6
            ...
            self.encoder = Encoder(c(position), EncoderLayer(d_model, c(attn), c(ff), dropout), N)
            self.encoder_linear = nn.Linear(args.embed_size, args.hidden_size, bias=False)
        elif args.lstm == 'lstm':
            self.encoder = nn.LSTM(args.embed_size, int(args.hidden_size / 2), bidirectional=True)
    ...
    def encode_transformer(self, src_sents_var, src_sents_len):
        """Encode the input natural language utterance
        ...
        last_state = torch.mean(src_encodings, 1)
        last_cell = torch.mean(src_encodings, 1)
    ...
    def score(self, examples, return_encode_state=False):
        ...
        if self.args.encoder == 'transformer':
            src_encodings, (last_state, last_cell) = self.encode_transformer(batch.src_sents_var, batch.src_sents_len)
        else:
