...
def preprocess_conala_dataset(train_file, mined_file, test_file, grammar_file, src_freq=3, code_freq=3):
    np.random.seed(1234)
    ...
    train_examples, train_example_ids = preprocess_dataset(train_file, name='train', transition_system=transition_system)
    mined_examples = preprocess_mined_dataset(mined_file, num_examples=1000, transition_system=transition_system, train_example_ids=train_example_ids)
    # held out 200 examples for development
    full_train_examples = train_examples[:]
    np.random.shuffle(train_examples)
    dev_examples = train_examples[:200]
    train_examples.extend(mined_examples)
...
def preprocess_dataset(file_path, transition_system, name="train", dataset=None):
    if dataset is None:
    ...
    for i, example_json in enumerate(dataset):
        ...
        question_ids.append(example_json['question_id'])
    ...
    if name == "train":
        return examples, question_ids
    else:
        return examples


def preprocess_mined_dataset(file_path, num_examples, transition_system, train_example_ids):
    dataset = []
    count = 0
    with open(file_path) as f:
        for line_num, line in enumerate(f):
            example_json = json.loads(line)
            if example_json["question_id"] not in train_example_ids:
                dataset.append(example_json)
                count += 1
            if count == num_examples:
                print("Mined example number", line_num)
                break

    examples = preprocess_dataset(file_path, transition_system, "mined", dataset)
...
if __name__ == '__main__':
    ...
    preprocess_conala_dataset(train_file='data/conala/conala-train.json',
                              mined_file='data/conala/conala-mined.jsonl',
